  ...
  useFrame((state) => {
    if (mesh.current) {
      ...
      mesh.current.rotation.y = THREE.MathUtils.lerp(
        ...
        (mouse.current.x / viewport.width) * 0.5 + state.clock.getElapsedTime() * 0.1,
        0.1
      );
  ...
  return (
    <>
      <Float
        ...
        floatIntensity={0.5}
        position={new THREE.Vector3(0, isMobile ? 0 : 0.5, 0)}
      >
        <mesh ref={mesh} scale={new THREE.Vector3(scale[0], scale[1], scale[2])}>
        ...
        {/* Particules brillantes autour de la sphère */}
        <Sparkles
          count={50}
          scale={4}
          size={2}
  ...
  return (
    <div className="h-screen w-full relative overflow-hidden">
      <Canvas
        ...
        camera={{ position: new THREE.Vector3(0, 0, isMobile ? 4 : 5), fov: 45 }}
      >
        <PerspectiveCamera
          makeDefault
          position={new THREE.Vector3(0, 0, isMobile ? 4 : 5)}
          fov={45}
        />
        ...
        <spotLight
          ...
          color="#4338ca"
        />
        <Scene mouse={mouse} isMobile={isMobile} scrollY={scrollY} />
